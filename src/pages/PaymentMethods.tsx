...
import { Plus, Edit3, Trash2, ToggleRight, GripVertical } from 'lucide-react';
...
import { PAYMENT_TYPE_LABELS, PAYMENT_TYPE_ICONS, BANK_LABELS } from '../utils/helpers';
...
export default function PaymentMethods() {
  const {
    data, addPaymentMethod, updatePaymentMethod,
  ...
  // Drag-to-reorder acts on the full paymentMethods array (active only reordering)
  // We map active indices back to full-array indices
  const activeIndices = data.paymentMethods
    .map((pm, i) => ({ pm, i }))
    .filter(({ pm }) => pm.active)
    .map(({ i }) => i);

  const drag = useDragSort(activeMethods, (fromActive, toActive) => {
    const fromFull = activeIndices[fromActive];
    const toFull   = activeIndices[toActive];
    if (fromFull !== undefined && toFull !== undefined) {
      reorderPaymentMethods(fromFull, toFull);
    }
  });
  ...
  const openEdit = (pm: PaymentMethod) => {
    setForm({
      type: pm.type, bankCode: (pm.bankCode as BankCode) ?? 'hsbc',
      bankName: pm.bankName ?? '', label: pm.label,
      last4: pm.last4 ?? '', autopayEnabled: pm.autopayEnabled,
      note: pm.note ?? '', active: pm.active,
    });
    setEditingId(pm.id); setErrors({}); setModalOpen(true);
  };
  ...
  const handleSave = () => {
    ...
    if (editingId) updatePaymentMethod(editingId, payload);
    else addPaymentMethod(payload);
    setModalOpen(false);
  };

  const handleDelete = (pm: PaymentMethod) => {
    if (confirm(`確定要刪除「${pm.label}」付款方法？`)) deletePaymentMethod(pm.id);
  };
  ...
  const getPaidAmountThisMonth = (pmId: string) => {
    ...
    return data.billRecords
      .filter((r) => r.period === p && r.paid && r.paymentMethodId === pmId)
  ...
  return (
    <div className="p-4 lg:p-6 max-w-4xl mx-auto space-y-4">
      <div className="flex items-center justify-between">
        ...
        <Button size="sm" onClick={openAdd}><Plus size={14} /> 新增</Button>
      </div>

      {/* Active — draggable */}
      <div className="space-y-2" {...drag.containerProps}>
        {activeMethods.map((pm, activeIdx) => {
          const usage   = getUsageCount(pm.id);
          const paidAmt = getPaidAmountThisMonth(pm.id);
          return (
            <div key={pm.id} {...drag.itemProps(activeIdx)} style={drag.itemStyle(activeIdx)}>
              <Card>
                <CardBody>
                  <div className="flex items-center gap-3">
                    ...
                    />
                    <div className="w-11 h-11 rounded-xl bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-xl flex-shrink-0">
                      {PAYMENT_TYPE_ICONS[pm.type]}
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 flex-wrap">
                        <p className="font-semibold text-slate-900 dark:text-white">{pm.label}</p>
                        {pm.last4 && <span className="text-xs text-slate-400 font-mono">···· {pm.last4}</span>}
                    ...
                    <div className="flex items-center gap-1.5">
                      <button
                        ...
                        title="停用"
                      >
                        <ToggleRight size={17} className="text-indigo-600" />
                      </button>
                      <button
                        onClick={() => openEdit(pm)}
                        className="p-1.5 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-400"
      ...
      {/* Modal */}
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editingId ? '編輯付款方法' : '新增付款方法'}>
        <div className="space-y-4">
          <FormField label="付款類型" required>
            <Select value={form.type} onChange={(e) => setForm({ ...form, type: e.target.value as PaymentMethodType })}>
              {Object.entries(PAYMENT_TYPE_LABELS).map(([k, v]) => (
                <option key={k} value={k}>{PAYMENT_TYPE_ICONS[k as PaymentMethodType]} {v}</option>
              ))}
            </Select>
          </FormField>
          {(form.type === 'credit_card' || form.type === 'bank_transfer') && (
            <FormField label="銀行">
              <Select value={form.bankCode} onChange={(e) => setForm({ ...form, bankCode: e.target.value as BankCode })}>
          ...
          {(form.type === 'credit_card' || form.type === 'bank_transfer') && form.bankCode === 'other' && (
            <FormField label="銀行名稱">
              <Input value={form.bankName} onChange={(e) => setForm({ ...form, bankName: e.target.value })} placeholder="請輸入銀行名稱" />
            </FormField>
          )}
          <FormField label="顯示名稱" required error={errors.label}>
            <Input value={form.label} onChange={(e) => setForm({ ...form, label: e.target.value })} placeholder="例如：HSBC Red、轉數快、現金" error={!!errors.label} />
          </FormField>
          {form.type === 'credit_card' && (
            <FormField label="最後四位數字" hint="可選，方便識別">
              <Input value={form.last4} onChange={(e) => setForm({ ...form, last4: e.target.value.slice(0, 4) })} placeholder="1234" maxLength={4} />
            </FormField>
          )}
          <FormField label="AutoPay 設定">
            <label className="flex items-center gap-2 cursor-pointer">
              <input type="checkbox" checked={form.autopayEnabled} onChange={(e) => setForm({ ...form, autopayEnabled: e.target.checked })} className="w-4 h-4 rounded accent-indigo-600" />
              <span className="text-sm text-slate-700 dark:text-slate-300">此付款方法已設定自動轉賬 (AutoPay)</span>
            </label>
          </FormField>
          <FormField label="備註">
            <TextArea value={form.note} onChange={(e) => setForm({ ...form, note: e.target.value })} rows={2} placeholder="可選備註" />
          </FormField>
          <div className="flex gap-3">
            <Button onClick={handleSave} className="flex-1">{editingId ? '儲存更改' : '新增付款方法'}</Button>
